...
export default function ScanForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const apiUrl = `/api/scan?url=${encodeURIComponent(url)}`;
      ...
      if (!res.ok) {
        ...
        throw new Error(
          data.error || `Scan failed with status ${res.status}`
        );
      }

      if (!data.lighthouseResult?.categories) {
        console.error(
          "❌ NO LIGHTHOUSE CATEGORIES FOUND:",
          data
        );
    ...
    <div className="min-h-screen bg-[#E0DAD0]">
      ...
      <div className="max-w-[1200px] mx-auto p-4">
        <form
          ...
          className="flex flex-col items-center gap-6 md:max-w-2/3 m-auto"
        >
          <input
            type="text"
            placeholder="Enter URL  [e.g : https://apple.com]"
            value={url}
        ...
        {result && (
          ...
          >
            ...
            <div className="md:mt-6">
              <h2 className="text-xl text-center py-6 font-semibold">
                Metrics:
              </h2>
